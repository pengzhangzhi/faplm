...
class ProGenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.scale_attn = torch.sqrt(torch.tensor(self.head_dim, dtype=torch.float16)).to(torch.get_default_dtype())
...
class ProGenBlock(nn.Module):
    def __init__(self, config):
        ...
        self.attn = ProGenFlashAttention2(config) if FLASH_ATTN_AVAILABLE else ProGenAttention(config)
