...
@pytest.mark.parametrize("model_version,batch_size,dtype,max_seq_lengths,repeats", [
    ("facebook/esm2_t33_650M_UR50D", 8, torch.float16, [100, 200, 300, 400, 500, 600, 700, 800, 1000], 10)
])
def test_esm_vs_faesm_benchmark(model_version, batch_size, dtype, max_seq_lengths, repeats):
    ...
    fa_esm = FAEsmForMaskedLM.from_pretrained(model_version, use_fa=True).to(device).to(dtype).eval()
    ...
    for seq_length in max_seq_lengths:
        ...
        print(f"Seq Len: {seq_length}, Avg ESM Mem: {esm_memory_usage[-1]:.3f} GB, Avg FAESM Mem: {fa_esm_memory_usage[-1]:.3f} GB")
        print(f"Seq Len: {seq_length}, Avg ESM Time: {esm_inference_times[-1]:.3f} s, Avg FAESM Time: {fa_esm_inference_times[-1]:.3f} s")
    ...
    memory_reduction = [(1 - (fa / esm)) * 100 for fa, esm in zip(fa_esm_memory_usage, esm_memory_usage)]
    time_reduction = [(1 - (fa / esm)) * 100 for fa, esm in zip(fa_esm_inference_times, esm_inference_times)]

    fig, axes = plt.subplots(1, 2, figsize=(20, 8))  # Larger figure for better resolution

    # Left Plot: Memory Benchmark
    ax1 = axes[0]
    ax1.plot(max_seq_lengths, esm_memory_usage, label='ESM Memory Usage (GB)', marker='o', color=color_palette[0])
    ax1.plot(max_seq_lengths, fa_esm_memory_usage, label='FAESM Memory Usage (GB)', marker='o', color=color_palette[1])
    ...
    ax1_twin = ax1.twinx()
    ax1_twin.plot(max_seq_lengths, memory_reduction, label='Memory Reduction (%)', marker='o', linestyle='--', color=color_palette[2])
    ...
    ax2 = axes[1]
    ax2.plot(max_seq_lengths_filtered, esm_inference_times, label='ESM Inference Time (s)', marker='o', color=color_palette[0])
    ax2.plot(max_seq_lengths_filtered, fa_esm_inference_times, label='FAESM Inference Time (s)', marker='o', color=color_palette[1])
    ...
    ax2_twin = ax2.twinx()
    ax2_twin.plot(max_seq_lengths_filtered, time_reduction, label='Time Reduction (%)', marker='o', linestyle='--', color=color_palette[2])
    ...
    plt.suptitle(f"Model Version: {model_version}\nBatch Size: {batch_size}, Data Type: {dtype}, Averaged over {repeats} runs", fontsize=20)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig('benchmark.png', dpi=300)  # High resolution
    plt.close()

    for seq_length, fa_mem, esm_mem, fa_time, esm_time in zip(max_seq_lengths, fa_esm_memory_usage, esm_memory_usage, fa_esm_inference_times, esm_inference_times):
        assert fa_mem <= esm_mem, f"Seq {seq_length}: FAESM {fa_mem:.3f} GB > ESM {esm_mem:.3f} GB!"
        assert fa_time <= esm_time, f"Seq {seq_length}: FAESM {fa_time:.3f} s > ESM {esm_time:.3f} s!"